...
class Atur extends CI_Controller{
	...
	public function proseslogin(){
        ...
        $q_cek  = $this->db->query("SELECT * FROM user WHERE username = '".$u."' AND password = '".$p."'");
	...
	public function buku(){
		...
		if($ke=="tambah"){
			...
			$this->MBuku->insertData($data);
                //$this->db->query("INSERT INTO buku VALUES ('".$this->input->post('kdbuku')."', '".$this->input->post('judul')."', '".$this->input->post('pengarang')."', '".$this->input->post('harga')."','".$this->input->post('stock')."')");
        	...
        	$this->db->query("DELETE from buku where kdbuku='$code'");
            $this->session->set_flashdata("k", "<div class=\"alert alert-danger\">Buku berhasil dihapus</div>");
            redirect('atur/buku');
    	}else if($ke=="edit"){
                //$this->db->query("UPDATE buku set kdbuku='".$this->input->post('kdbuku')."', judul='".$this->input->post('judul')."', pengarang='".$this->input->post('pengarang')."', harga='".$this->input->post('harga')."',stock='".$this->input->post('stock')."') where kdbuku='".$this->input->post('idedit')."'");
    		$data = array(
				'kdbuku' => $this->input->post('kdbuku'),
				'judul' => $this->input->post('judul'),
				'pengarang' => $this->input->post('pengarang'),
